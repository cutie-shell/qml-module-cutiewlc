...
void CutieAppThumbnail::setWlc(CutieShell *wlc) {
    if (m_wlc == wlc) return;
    m_wlc = wlc;
    connect(m_wlc, &CutieShell::thumbnailDamage, this, &CutieAppThumbnail::onThumbnailDamage);
...
void CutieAppThumbnail::newFrame() {
    ...
    connect(m_frame, &ScreencopyFrameV1::ready, this, &CutieAppThumbnail::onReady);
    connect(m_frame, &ScreencopyFrameV1::failed, this, &CutieAppThumbnail::onFailed);
}
...
void CutieAppThumbnail::onThumbnailDamage(void *object) {
    ...
    ForeignToplevelHandleV1 *handle = m_toplevel->manager()->handleFromObject(object);
